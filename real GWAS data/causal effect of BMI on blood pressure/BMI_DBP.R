...
for(i in 1:length(d)){
  IVuv[[i]]=exp[[i]]$snp[exp[[i]]$snp%in%indsnp&exp[[i]]$p<5e-8]
  IV[[i]]=c(exp[[i]]$snp[exp[[i]]$snp%in%indsnp&exp[[i]]$p<5e-8])
  for(j in 1:d[[i]]){
    IV[[i]]=c(IV[[i]],cov[[i]][[j]]$snp[cov[[i]][[j]]$snp%in%indsnp&cov[[i]][[j]]$p<5e-8])
  }
...
for(i in 1:length(d)){
  ...
  beta_out[[i]]=DBP$beta[DBP$snp%in%IV[[i]]]
  se_out[[i]]=DBP$se[DBP$snp%in%IV[[i]]]
}
correlation=list()
correlation_uv=list()
...
for(i in 1:length(d)){
  ...
  nullz[[i]]=cbind(nullz[[i]],DBP$z[null_z])
...
for(i1 in 1:length(d)){
  ...
  for(i2 in 1:DP){
    ...
    cmlmv=MVmr_cML(b_exp =beta_expDP,b_out=beta_outDP,se_bx = se_expDP,Sig_inv_l = SIGinvDP,
                   n=100000,K_vec = c(0:(niv-dDP-2)),random_start = 1,min_theta_range = 0.1,
                   max_theta_range = 0.1,maxit = 500,thres = 0.01) 
    
    theta_cmlDP[i2]=cmlmv$BIC_theta[1]
    invalidmv[[i2]]=cmlmv$BIC_invalid
    cmluv=MVmr_cML(b_exp =beta_expDP_uv,b_out=matrix(beta_outDP_uv,ncol=1,nrow=length(beta_outDP_uv)),se_bx = matrix(se_expDP_uv,ncol = 1,nrow=length(se_expDP_uv)),Sig_inv_l = SIGinvDP_uv,
             n=100000,K_vec = c(0:(niv_uv-2)),random_start = 1,min_theta_range = 0.1,
             max_theta_range = 0.1,maxit = 500,thres = 0.01)
    theta_cmluvDP[i2]=cmluv$BIC_theta[1]
    invaliduv[[i2]]=cmluv$BIC_invalid
...
save(output,file="BMI_DBP.RData")
input=list()
for(i in 1:length(d)){
