setwd("D:/paper2/simulation/mediator_H/all_valid")
N_non_null=40
N_H_only=40
N_null=3000
DP=10 
...
input_file=c(
  "mediator_Hdim=1_no_pleiotropy.RData",
  "mediator_Hdim=2_no_pleiotropy.RData",
  "mediator_Hdim=4_no_pleiotropy.RData" 
)
...
for(i1 in 1:length(input_file)){
  load(input_file[i1])
  betaGU=as.matrix(effects_mediator_H$betaGU,nrow=Nsnps) 
  betaGH=as.matrix(effects_mediator_H$betaGH,nrow=Nsnps) 
  betaGX=as.matrix(effects_mediator_H$betaGX,nrow=Nsnps)
  
  betaGY=as.matrix(effects_mediator_H$betaGY,nrow=Nsnps) 
  betaHY=as.matrix(effects_mediator_H$betaHY,ncol=1) 
  betaUX=effects_mediator_H$betaUX 
  betaUY=effects_mediator_H$betaUY 
  d=nrow(betaHY)
  betaUH=matrix(effects_mediator_H$betaUH,ncol=d,nrow=1) 
  betaXH=matrix(effects_mediator_H$betaXH,ncol=d,nrow=1) 
  theta=effects_mediator_H$theta
...
X_known=Gmatrix%*%betaGX+U%*%betaUX 
VarEX=(0.1/0.9)*var(X_known)
X=X_known+rnorm(n=sample_size,mean=0,sd=sqrt(1))
H_known=Gmatrix%*%betaGH+U%*%betaUH+X%*%betaXH
VarEH=numeric()
for(i in 1:d){
  VarEH[i]=var(H_known[,i])*(0.1/0.9)
}
...
betaXH=t(betaXH)
betaUH=t(betaUH)
varEX=(t(betaGX)%*%covG%*%betaGX)[1,1]+1
covX=(t(betaGX)%*%covG%*%betaGX+betaUX^2*VarEU+1)[1,1]
CovEH=t(betaGH)%*%covG%*%betaGH +diag(1,nrow = d,ncol = d) 
M2=solve((betaXH%*%t(betaXH)*betaUX^2+betaUH%*%t(betaUH)+betaUX*(betaXH%*%t(betaUH)+betaUH%*%t(betaXH)))*VarEU+betaXH%*%t(betaXH)*varEX+CovEH)
b_X_true=as.vector(-betaUX*VarEU*M2%*%(betaUX*betaXH+betaUH))
b_Y_true=as.vector(-(betaUY+theta*betaUX)*VarEU*M2%*%(betaUX*betaXH+betaUH))
...
for(i in 1:DP){
  ...
  est_mv_cml_no_adj=MVmr_cML(b_exp =data_mv_no_adj[,1:(ncol(data_mv_no_adj)-1)],b_out =as.matrix(data_mv_no_adj[,ncol(data_mv_no_adj)],nrow=N_non_null,ncol=1),se_bx =se_exp_no_adj,Sig_inv_l = SIG_inv_no_adjust,n=sample_size,K_vec =c(0:(N_non_null-d-2)),random_start = d+1,min_theta_range = 0.2,max_theta_range = 0.2,thres = 0.01   )
  est_mv_cml_X_adj=MVmr_cML(b_exp =data_mv_X_adj[,1:(ncol(data_mv_X_adj)-1)],b_out =as.matrix(data_mv_X_adj[,ncol(data_mv_X_adj)],nrow=N_non_null,ncol=1),se_bx =se_exp_adj,Sig_inv_l = SIG_inv_X_adjust,n=sample_size,K_vec =c(0:(N_non_null-d-2)),random_start = d+1,min_theta_range = 0.2,max_theta_range = 0.2,thres = 0.01   )
  est_mv_cml_Y_adj=MVmr_cML(b_exp =data_mv_Y_adj[,1:(ncol(data_mv_Y_adj)-1)],b_out =as.matrix(data_mv_Y_adj[,ncol(data_mv_Y_adj)],nrow=N_non_null,ncol=1),se_bx =se_exp_adj,Sig_inv_l = SIG_inv_Y_adjust,n=sample_size,K_vec =c(0:(N_non_null-d-2)),random_start = d+1,min_theta_range = 0.2,max_theta_range = 0.2,thres = 0.01   )
  est_mv_cml_X_and_Y_adj=MVmr_cML(b_exp =data_mv_X_and_Y_adj[,1:(ncol(data_mv_X_and_Y_adj)-1)],b_out =as.matrix(data_mv_X_and_Y_adj[,ncol(data_mv_X_and_Y_adj)],nrow=N_non_null,ncol=1),se_bx =se_exp_adj,Sig_inv_l = SIG_inv_X_and_Y_adjust,n=sample_size,K_vec =c(0:(N_non_null-d-2)),random_start = d+1,min_theta_range = 0.2,max_theta_range = 0.2,thres = 0.01   )
  est_uv_cml_no_adj=MVmr_cML(b_exp =as.matrix(data_uv_no_adj[,1:(ncol(data_uv_no_adj)-1)],nrow=N_non_null,ncol=1),b_out =as.matrix(data_uv_no_adj[,ncol(data_uv_no_adj)],nrow=N_non_null,ncol=1),se_bx =se_exp_no_adj,Sig_inv_l = SIG_inv_uv_no_adjust,n=sample_size,K_vec =c(0:(N_non_null-2)),random_start = 1,min_theta_range = 0.2,max_theta_range = 0.2,thres = 0.01   )
  est_uv_cml_X_adj=MVmr_cML(b_exp =as.matrix(data_uv_X_adj[,1:(ncol(data_uv_X_adj)-1)],nrow=N_non_null,ncol=1),b_out =as.matrix(data_uv_X_adj[,ncol(data_uv_X_adj)],nrow=N_non_null,ncol=1),se_bx =se_exp_adj,Sig_inv_l = SIG_inv_uv_X_adjust,n=sample_size,K_vec =c(0:(N_non_null-2)),random_start = 1,min_theta_range = 0.2,max_theta_range = 0.2,thres = 0.01   )
  est_uv_cml_Y_adj=MVmr_cML(b_exp =as.matrix(data_uv_Y_adj[,1:(ncol(data_uv_Y_adj)-1)],nrow=N_non_null,ncol=1),b_out =as.matrix(data_uv_Y_adj[,ncol(data_uv_Y_adj)],nrow=N_non_null,ncol=1),se_bx =se_exp_adj,Sig_inv_l = SIG_inv_uv_Y_adjust,n=sample_size,K_vec =c(0:(N_non_null-2)),random_start = 1,min_theta_range = 0.2,max_theta_range = 0.2,thres = 0.01  )
  est_uv_cml_X_and_Y_adj=MVmr_cML(b_exp =as.matrix(data_uv_X_and_Y_adj[,1:(ncol(data_uv_X_and_Y_adj)-1)],nrow=N_non_null,ncol=1),b_out =as.matrix(data_uv_X_and_Y_adj[,ncol(data_uv_X_and_Y_adj)],nrow=N_non_null,ncol=1),se_bx =se_exp_adj,Sig_inv_l = SIG_inv_uv_X_and_Y_adjust,n=sample_size,K_vec =c(0:(N_non_null-2)),random_start = 1,min_theta_range = 0.2,max_theta_range = 0.2,thres = 0.01  )
...
theta_mv_cml_no_adj=colMeans(est_mv_cml_no_adj_DP)
se_theta_mv_cml_no_adj=sd(est_mv_cml_no_adj_DP)
theta_mv_ivw_no_adj=colMeans(est_mv_ivw_no_adj_DP)
se_theta_mv_ivw_no_adj=sd(est_mv_ivw_no_adj_DP)
theta_mv_lasso_no_adj=colMeans(est_mv_lasso_no_adj_DP)
se_theta_mv_lasso_no_adj=sd(est_mv_lasso_no_adj_DP)
theta_mv_median_no_adj=colMeans(est_mv_median_no_adj_DP)
se_theta_mv_median_no_adj=sd(est_mv_median_no_adj_DP)
theta_mv_egger_no_adj=colMeans(est_mv_egger_no_adj_DP)
se_theta_mv_egger_no_adj=sd(est_mv_egger_no_adj_DP)
theta_mv_cml_X_adj=colMeans(est_mv_cml_X_adj_DP)
se_theta_mv_cml_X_adj=sd(est_mv_cml_X_adj_DP)
theta_mv_ivw_X_adj=colMeans(est_mv_ivw_X_adj_DP)
se_theta_mv_ivw_X_adj=sd(est_mv_ivw_X_adj_DP)
theta_mv_lasso_X_adj=colMeans(est_mv_lasso_X_adj_DP)
se_theta_mv_lasso_X_adj=sd(est_mv_lasso_X_adj_DP)
theta_mv_median_X_adj=colMeans(est_mv_median_X_adj_DP)
se_theta_mv_median_X_adj=sd(est_mv_median_X_adj_DP)
theta_mv_egger_X_adj=colMeans(est_mv_egger_X_adj_DP)
se_theta_mv_egger_X_adj=sd(est_mv_egger_X_adj_DP)
theta_mv_cml_Y_adj=colMeans(est_mv_cml_Y_adj_DP)
se_theta_mv_cml_Y_adj=sd(est_mv_cml_Y_adj_DP)
theta_mv_ivw_Y_adj=colMeans(est_mv_ivw_Y_adj_DP)
se_theta_mv_ivw_Y_adj=sd(est_mv_ivw_Y_adj_DP)
theta_mv_lasso_Y_adj=colMeans(est_mv_lasso_Y_adj_DP)
se_theta_mv_lasso_Y_adj=sd(est_mv_lasso_Y_adj_DP)
theta_mv_median_Y_adj=colMeans(est_mv_median_Y_adj_DP)
se_theta_mv_median_Y_adj=sd(est_mv_median_Y_adj_DP)
theta_mv_egger_Y_adj=colMeans(est_mv_egger_Y_adj_DP)
se_theta_mv_egger_Y_adj=sd(est_mv_egger_Y_adj_DP)
theta_mv_cml_X_and_Y_adj=colMeans(est_mv_cml_X_and_Y_adj_DP)
se_theta_mv_cml_X_and_Y_adj=sd(est_mv_cml_X_and_Y_adj_DP)
theta_mv_ivw_X_and_Y_adj=colMeans(est_mv_ivw_X_and_Y_adj_DP)
se_theta_mv_ivw_X_and_Y_adj=sd(est_mv_ivw_X_and_Y_adj_DP)
theta_mv_lasso_X_and_Y_adj=colMeans(est_mv_lasso_X_and_Y_adj_DP)
se_theta_mv_lasso_X_and_Y_adj=sd(est_mv_lasso_X_and_Y_adj_DP)
theta_mv_median_X_and_Y_adj=colMeans(est_mv_median_X_and_Y_adj_DP)
se_theta_mv_median_X_and_Y_adj=sd(est_mv_median_X_and_Y_adj_DP)
theta_mv_egger_X_and_Y_adj=colMeans(est_mv_egger_X_and_Y_adj_DP)
se_theta_mv_egger_X_and_Y_adj=sd(est_mv_egger_X_and_Y_adj_DP)
theta_uv_cml_no_adj=colMeans(est_uv_cml_no_adj_DP)
se_theta_uv_cml_no_adj=sd(est_uv_cml_no_adj_DP)
theta_uv_ivw_no_adj=colMeans(est_uv_ivw_no_adj_DP)
se_theta_uv_ivw_no_adj=sd(est_uv_ivw_no_adj_DP)
theta_uv_lasso_no_adj=colMeans(est_uv_lasso_no_adj_DP)
se_theta_uv_lasso_no_adj=sd(est_uv_lasso_no_adj_DP)
theta_uv_median_no_adj=colMeans(est_uv_median_no_adj_DP)
se_theta_uv_median_no_adj=sd(est_uv_median_no_adj_DP)
theta_uv_egger_no_adj=colMeans(est_uv_egger_no_adj_DP)
se_theta_uv_egger_no_adj=sd(est_uv_egger_no_adj_DP)
theta_uv_cml_X_adj=colMeans(est_uv_cml_X_adj_DP)
se_theta_uv_cml_X_adj=sd(est_uv_cml_X_adj_DP)
theta_uv_ivw_X_adj=colMeans(est_uv_ivw_X_adj_DP)
se_theta_uv_ivw_X_adj=sd(est_uv_ivw_X_adj_DP)
theta_uv_lasso_X_adj=colMeans(est_uv_lasso_X_adj_DP)
se_theta_uv_lasso_X_adj=sd(est_uv_lasso_X_adj_DP)
theta_uv_median_X_adj=colMeans(est_uv_median_X_adj_DP)
se_theta_uv_median_X_adj=sd(est_uv_median_X_adj_DP)
theta_uv_egger_X_adj=colMeans(est_uv_egger_X_adj_DP)
se_theta_uv_egger_X_adj=sd(est_uv_egger_X_adj_DP)
theta_uv_cml_Y_adj=colMeans(est_uv_cml_Y_adj_DP)
se_theta_uv_cml_Y_adj=sd(est_uv_cml_Y_adj_DP)
theta_uv_ivw_Y_adj=colMeans(est_uv_ivw_Y_adj_DP)
se_theta_uv_ivw_Y_adj=sd(est_uv_ivw_Y_adj_DP)
theta_uv_lasso_Y_adj=colMeans(est_uv_lasso_Y_adj_DP)
se_theta_uv_lasso_Y_adj=sd(est_uv_lasso_Y_adj_DP)
theta_uv_median_Y_adj=colMeans(est_uv_median_Y_adj_DP)
se_theta_uv_median_Y_adj=sd(est_uv_median_Y_adj_DP)
theta_uv_egger_Y_adj=colMeans(est_uv_egger_Y_adj_DP)
se_theta_uv_egger_Y_adj=sd(est_uv_egger_Y_adj_DP)
theta_uv_cml_X_and_Y_adj=colMeans(est_uv_cml_X_and_Y_adj_DP)
se_theta_uv_cml_X_and_Y_adj=sd(est_uv_cml_X_and_Y_adj_DP)
theta_uv_ivw_X_and_Y_adj=colMeans(est_uv_ivw_X_and_Y_adj_DP)
se_theta_uv_ivw_X_and_Y_adj=sd(est_uv_ivw_X_and_Y_adj_DP)
theta_uv_lasso_X_and_Y_adj=colMeans(est_uv_lasso_X_and_Y_adj_DP)
se_theta_uv_lasso_X_and_Y_adj=sd(est_uv_lasso_X_and_Y_adj_DP)
theta_uv_median_X_and_Y_adj=colMeans(est_uv_median_X_and_Y_adj_DP)
se_theta_uv_median_X_and_Y_adj=sd(est_uv_median_X_and_Y_adj_DP)
theta_uv_egger_X_and_Y_adj=colMeans(est_uv_egger_X_and_Y_adj_DP)
...
if(d==1){
  ...
  library(indexevent)
    ...
    for(i in 1:DP){
      ...
      b_DP_X_cml[i,]=MVmr_cML(b_exp =H_exp_DP,b_out = as.matrix(X_out_DP),se_bx = H_exp_se,Sig_inv_l = cov_invHX,n=sample_size,random_start = d,min_theta_range = -0.1,max_theta_range = -0.1,maxit = 200,thres = 0.01 )$BIC_theta
      ...
      b_DP_Y_cml[i,]=MVmr_cML(b_exp =H_exp_DP,b_out = as.matrix(Y_out_DP),se_bx = H_exp_se,Sig_inv_l = cov_invHY,n=sample_size,random_start = d,min_theta_range = -0.1,max_theta_range = -0.1,maxit = 200,thres = 0.01 )$BIC_theta
  ...
  for(i in 1:DP){
    ...
    theta_corrected_cml_cml_DP[i,]=MVmr_cML(b_exp=as.matrix(X_DP_cml),b_out =as.matrix(Y_DP_cml),se_bx = as.matrix(seX_cml),Sig_inv_l = SIGinv_cml,n=sample_size,K_vec = c(0:(N_non_null-2)),random_start = 1,min_theta_range = 0.2,max_theta_range = 0.2,thres = 0.01 )$BIC_theta
  }
  theta_corrected_cml_cml=colMeans(theta_corrected_cml_cml_DP)
  theta_corrected_cml_cml_se=sd(theta_corrected_cml_cml_DP)
  theta_corrected_ivw_cml=colMeans(theta_corrected_ivw_cml_DP)
  theta_corrected_ivw_cml_se=sd(theta_corrected_ivw_cml_DP)
  theta_corrected_lasso_cml=colMeans(theta_corrected_lasso_cml_DP)
  theta_corrected_lasso_cml_se=sd(theta_corrected_lasso_cml_DP)
  theta_corrected_egger_cml=colMeans(theta_corrected_egger_cml_DP)
  theta_corrected_egger_cml_se=sd(theta_corrected_egger_cml_DP)
  theta_corrected_median_cml=colMeans(theta_corrected_median_cml_DP)
  ...
  for(i in 1:DP){
    ...
    theta_corrected_cml_ivw_DP[i,]=MVmr_cML(b_exp=as.matrix(X_DP_ivw),b_out =as.matrix(Y_DP_ivw),se_bx = as.matrix(seX_ivw),Sig_inv_l = SIGinv_ivw,n=sample_size,K_vec = c(0:(N_non_null-2)),random_start = 1,min_theta_range = 0.2,max_theta_range = 0.2,thres = 0.01 )$BIC_theta
  }
  
  
  
  theta_corrected_cml_ivw=colMeans(theta_corrected_cml_ivw_DP)
  theta_corrected_cml_ivw_se=sd(theta_corrected_cml_ivw_DP)
  theta_corrected_ivw_ivw=colMeans(theta_corrected_ivw_ivw_DP)
  theta_corrected_ivw_ivw_se=sd(theta_corrected_ivw_ivw_DP)
  
  theta_corrected_lasso_ivw=colMeans(theta_corrected_lasso_ivw_DP)
  theta_corrected_lasso_ivw_se=sd(theta_corrected_lasso_ivw_DP)
  theta_corrected_egger_ivw=colMeans(theta_corrected_egger_ivw_DP)
  theta_corrected_egger_ivw_se=sd(theta_corrected_egger_ivw_DP)
  theta_corrected_median_ivw=colMeans(theta_corrected_median_ivw_DP)
  ...
  for(i in 1:DP){
    ...
    theta_corrected_cml_dho_DP[i,]=MVmr_cML(b_exp=as.matrix(X_DP_dho),b_out =as.matrix(Y_DP_dho),se_bx = as.matrix(seX_dho),Sig_inv_l = SIGinv_dho,n=sample_size,K_vec = c(0:(N_non_null-2)),random_start = 1,min_theta_range = 0.2,max_theta_range = 0.2,thres = 0.01 )$BIC_theta
  }
  
  
  
  theta_corrected_cml_dho=colMeans(theta_corrected_cml_dho_DP)
  theta_corrected_cml_dho_se=sd(theta_corrected_cml_dho_DP)
  theta_corrected_ivw_dho=colMeans(theta_corrected_ivw_dho_DP)
  theta_corrected_ivw_dho_se=sd(theta_corrected_ivw_dho_DP)
  
  theta_corrected_lasso_dho=colMeans(theta_corrected_lasso_dho_DP)
  theta_corrected_lasso_dho_se=sd(theta_corrected_lasso_dho_DP)
  theta_corrected_egger_dho=colMeans(theta_corrected_egger_dho_DP)
  theta_corrected_egger_dho_se=sd(theta_corrected_egger_dho_DP)
  theta_corrected_median_dho=colMeans(theta_corrected_median_dho_DP)
  ...
  for(i in 1:DP){
    ...
    theta_corrected_cml_sh_DP[i,]=MVmr_cML(b_exp=as.matrix(X_DP_sh),b_out =as.matrix(Y_DP_sh),se_bx = as.matrix(seX_sh),Sig_inv_l = SIGinv_sh,n=sample_size,K_vec = c(0:(N_non_null-2)),random_start = 1,min_theta_range = 0.2,max_theta_range = 0.2,thres = 0.01 )$BIC_theta
  }
  
  
  
  theta_corrected_cml_sh=colMeans(theta_corrected_cml_sh_DP)
  theta_corrected_cml_sh_se=sd(theta_corrected_cml_sh_DP)
  theta_corrected_ivw_sh=colMeans(theta_corrected_ivw_sh_DP)
  theta_corrected_ivw_sh_se=sd(theta_corrected_ivw_sh_DP)
  
  theta_corrected_lasso_sh=colMeans(theta_corrected_lasso_sh_DP)
  theta_corrected_lasso_sh_se=sd(theta_corrected_lasso_sh_DP)
  theta_corrected_egger_sh=colMeans(theta_corrected_egger_sh_DP)
  theta_corrected_egger_sh_se=sd(theta_corrected_egger_sh_DP)
  theta_corrected_median_sh=colMeans(theta_corrected_median_sh_DP)
