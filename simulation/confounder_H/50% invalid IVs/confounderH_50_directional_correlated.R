...
for(i in 1:DP){
  ...
  est_mv_cml_no_adj=MVmr_cML(b_exp =data_mv_no_adj[,1:(ncol(data_mv_no_adj)-1)],b_out =as.matrix(data_mv_no_adj[,ncol(data_mv_no_adj)],nrow=N_non_null,ncol=1),se_bx =se_exp_no_adj,Sig_inv_l = SIG_inv_no_adjust,n=sample_size,K_vec =c(0:(N_non_null-d-2)),random_start = d+1,min_theta_range = 0.2,max_theta_range = 0.2,thres = 0.04 ,maxit=500  )
  est_mv_cml_X_adj=MVmr_cML(b_exp =data_mv_X_adj[,1:(ncol(data_mv_X_adj)-1)],b_out =as.matrix(data_mv_X_adj[,ncol(data_mv_X_adj)],nrow=N_non_null,ncol=1),se_bx =se_exp_adj,Sig_inv_l = SIG_inv_X_adjust,n=sample_size,K_vec =c(0:(N_non_null-d-2)),random_start = d+1,min_theta_range = 0.2,max_theta_range = 0.2,thres = 0.04  ,maxit=500 )
  est_mv_cml_Y_adj=MVmr_cML(b_exp =data_mv_Y_adj[,1:(ncol(data_mv_Y_adj)-1)],b_out =as.matrix(data_mv_Y_adj[,ncol(data_mv_Y_adj)],nrow=N_non_null,ncol=1),se_bx =se_exp_adj,Sig_inv_l = SIG_inv_Y_adjust,n=sample_size,K_vec =c(0:(N_non_null-d-2)),random_start = d+1,min_theta_range = 0.2,max_theta_range = 0.2,thres = 0.04 ,maxit=500  )
  est_mv_cml_X_and_Y_adj=MVmr_cML(b_exp =data_mv_X_and_Y_adj[,1:(ncol(data_mv_X_and_Y_adj)-1)],b_out =as.matrix(data_mv_X_and_Y_adj[,ncol(data_mv_X_and_Y_adj)],nrow=N_non_null,ncol=1),se_bx =se_exp_adj,Sig_inv_l = SIG_inv_X_and_Y_adjust,n=sample_size,K_vec =c(0:(N_non_null-d-2)),random_start = d+1,min_theta_range = 0.2,max_theta_range = 0.2,thres = 0.04 ,maxit=500  )
