setwd("/panfs/jay/groups/20/panwei/wan01299/paper2/simulation/confounder_H/no_collider_bias/")
N_non_null=40
N_H_only=40
N_null=3000
DP=100
...
UX=Gmatrix%*%betaGU+rnorm(n=sample_size,mean=0,sd=1)
UH=Gmatrix%*%betaGU+rnorm(n=sample_size,mean=0,sd=1)
UY=Gmatrix%*%betaGU+rnorm(n=sample_size,mean=0,sd=1)
H_known=Gmatrix%*%betaGH+UH%*%betaUH
VarEH=numeric()
for(i in 1:d){
...
H=H_known+EH
X_known=Gmatrix%*%betaGX+UX%*%betaUX+H%*%betaHX
VarEX=(0.1/0.9)*var(X_known)
X=X_known+rnorm(n=sample_size,mean=0,sd=sqrt(1))
Y_known=Gmatrix%*%betaGY+H%*%betaHY+UY%*%betaUY+theta*X
